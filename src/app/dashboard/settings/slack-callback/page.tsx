...
export default function SlackCallbackPage() {
  ...
  useEffect(() => {
    async function completeConnection() {
      ...
      if (!connectCode) {
        setError('Slack did not return a connect code. Start the Connect Slack flow again.');
        return;
      }

      try {
        const response = await fetch('/api/integrations/slack/complete', {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error(data.error || 'Slack connection could not be completed.');
        }

        router.replace('/dashboard/settings?slack=connected');
      } catch (completionError) {
        setError(
          completionError instanceof Error
            ? completionError.message
            : 'Slack connection could not be completed.'
        );
      }
    }
  ...
  return (
    <div className="flex min-h-full items-center justify-center p-8">
      <div className="max-w-xl rounded-2xl border border-surface-300 bg-surface-100/60 p-6 text-center shadow-xl">
        <h1 className="text-xl font-semibold text-surface-900">Connecting Slack</h1>
        {error ? (
          <>
            <p className="mt-3 text-sm text-red-300">{error}</p>
            <button
              onClick={() => router.replace('/dashboard/settings?slack=error')}
          ...
          <p className="mt-3 text-sm text-surface-600">
            SecureDesk is finalizing the Connected Account flow with Auth0 Token Vault.
          </p>
        )}
      </div>
    </div>
